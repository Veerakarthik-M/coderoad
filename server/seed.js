...
async function runSeed() {
  ...
  const app3 = execute(`INSERT INTO applications (student_user_id, institution_id, status, date_of_birth, age, gender, guardian_name, aadhaar_number, address, place, postal_name, pincode, district, roll_no, course, department, year_of_study, semester, academic_year, route_from, route_to, distance_km, concession_category) VALUES (?, ?, 'issued', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [stu3, inst3, '2003-12-10', 23, 'Female', 'Rajan R', '5678-9012-3456', '8, Lake Road, Ernakulam', 'Ernakulam', 'Kalamassery', '682022', 'Ernakulam', '20MCA018', 'MCA', 'Computer Applications', '3rd Year', 'S6', '2026-27', 'Ernakulam North', 'CUSAT Kalamassery', 12, 'General']);
  const cred3Id = `ANV-2026-${String(app3).padStart(6, '0')}`;
  const jws3 = await signCredential({ cid: cred3Id, sid: '20MCA018', name: 'Meera R', inst: 'Cochin University of Science and Technology', instId: inst3, uid: stu3, route: 'Ernakulam North to CUSAT Kalamassery', from_stop: 'Ernakulam North', to_stop: 'CUSAT Kalamassery', km: 12, type: 'Student Concession', from: '2026-04-01', to: '2027-03-31', iss: 'KSRTC-ANAVANDI', iat: Math.floor(Date.now() / 1000) });
  execute(`INSERT INTO credentials (application_id, credential_id, jws_token, valid_from, valid_to) VALUES (?, ?, ?, ?, ?)`, [app3, cred3Id, jws3, '2026-04-01', '2027-03-31']);
